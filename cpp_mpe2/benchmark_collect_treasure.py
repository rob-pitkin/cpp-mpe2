"""Benchmark C++ vs Python PettingZoo CollectTreasure environments."""
...
from cpp_mpe2.collect_treasure.collect_treasure import parallel_env as cpp_parallel_env
...
NUM_TREASURES = 6
MAX_CYCLES = 25
AGENTS = [f"collector_{i}" for i in range(NUM_COLLECTORS)] + \
         [f"deposit_{i}" for i in range(NUM_DEPOSITS)]
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("PettingZoo CollectTreasure Environment Benchmark")
    print("=" * 60)
    print(f"Configuration: {n_resets} resets, {n_steps} steps, {n_episodes} episodes")
    print(f"Max cycles per episode: {MAX_CYCLES}")
    print(f"Collectors: {NUM_COLLECTORS}, Deposits: {NUM_DEPOSITS}, Treasures: {NUM_TREASURES}")
    print()

    cpp_factory = lambda: cpp_parallel_env(
        num_collectors=NUM_COLLECTORS, num_deposits=NUM_DEPOSITS,
        num_treasures=NUM_TREASURES, max_cycles=MAX_CYCLES
    )
    ...
    mpe2_factory = lambda: collect_treasure_v1.parallel_env(
        num_collectors=NUM_COLLECTORS, num_deposits=NUM_DEPOSITS,
        num_treasures=NUM_TREASURES, max_cycles=MAX_CYCLES
    )
