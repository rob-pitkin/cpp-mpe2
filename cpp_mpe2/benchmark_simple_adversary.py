"""Benchmark C++ vs Python PettingZoo SimpleAdversary environments.

Compares cpp-mpe2 wrapper (C++ backend) vs mpe2 (Python backend).
"""
...
from cpp_mpe2.simple_adversary import parallel_env as cpp_parallel_env
...
def benchmark_steps(env_factory, n_steps):
    ...
    for _ in range(n_steps):
        # SimpleAdversary with N=2 has 3 agents: adversary_0, agent_0, agent_1
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("PettingZoo SimpleAdversary Environment Benchmark")
    print("=" * 60)
    print(f"Configuration: {n_resets} resets, {n_steps} steps, {n_episodes} episodes")
    print(f"Max cycles per episode: {max_cycles}")
    print(f"Agents: 3 (1 adversary + 2 good agents), Landmarks: 2")
    print()

    # C++ wrapper benchmarks (cpp-mpe2 with C++ backend)
    cpp_factory = lambda: cpp_parallel_env(N=2, max_cycles=max_cycles, continuous_actions=False)
    print("cpp-mpe2 (C++ backend with Python wrapper):")
    ...
    mpe2_factory = lambda: simple_adversary_v3.parallel_env(N=2, max_cycles=max_cycles, continuous_actions=False)
